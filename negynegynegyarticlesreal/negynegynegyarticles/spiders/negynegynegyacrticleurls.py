...
class A24huacrticleurlsSpider(scrapy.Spider):
    ...
    curr = conn.cursor()
    urls=[]
    curr.execute("""SELECT url FROM 'negynegynegyUrl_tb' WHERE
                 url LIKE "%2018/01%" OR
                 url LIKE "%2018/02%" OR
                 url LIKE "%2018/03%" OR
                 url LIKE "%2018/04%" OR
    ...
    def parse(self, response):
        ...
        tags = ['']
        start_url = ['']
        p = response.css("#content-main p::text,p span::text,#content-main p a::text").extract()
        connection = response.css("#content-main p a, p span").xpath("@href").extract()
        tag = response.css(".tag-list a::text").extract()
